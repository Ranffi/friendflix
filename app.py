...
def remove(movie_id, type):
    ...
    movie = db.session.query(
        models.Entertainment).filter_by(id=movie_id).first()
    if type == 'current':
...
def receive_data():
    ...
    user_exist = db.session.query(
        models.User).filter_by(user_name=name).first()
    if user_exist:
    ...
    user = models.User(
        ...
        thumbnail='https://icon-library.com/images/default-profile-icon/default-profile-icon-24.jpg'
    )

    # For an easy way to demo friends
    # Add myself as first friend like Tom in Myspace
    if(user.user_name != 'ranffi'):
        ranffi = db.session.query(models.User).filter_by(
            user_name='ranffi').first()

        user.follow.append(ranffi)
        ranffi.follow.append(user)
...
def search():
    ...
    show_looking_for = db.session.query(
        models.Entertainment).filter_by(title=title).first()

    if(show_looking_for == None):
        movie_res = requests.get(
            url=f'{os.environ.get("movie_api")}/?apikey={os.environ.get("api_key")}&t={title}')
